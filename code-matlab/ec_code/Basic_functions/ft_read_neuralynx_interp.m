...
for i = 1:nchans
  ...
  % manishm edit 2023-05-30
  % Sometimes the ts are the same, this is because of imperfect blocks. So
  % whenever that happens, shift one of the timestamps a little forward to
  % maintain uniqueness

 assert(isempty(find(diff(ts) == 0)), 'Timestamps generated by ft_read_data are not unique');
