...
for iM  = 1:length(mice)
    ...
    for iS = 1:length(sid)
        ...
        cd(this_dir);
        % Skip this directory if 'all_peths.mat' exists
...
function doStuff
    ...
    out.shuf_peth_mean, out.shuf_zpeth_mean, ...
    out.shuf_peth_sd, out.shuf_zpeth_sd, ...
    out.tvec, out.is_opto] = deal([]);
    LoadExpKeys;
...
%     cfg.fc = ExpKeys.goodCell;
    ...
    if ~isempty(S_opto.label) % ~isempty(S_others.label)
        ...
        for iC = 1:length(S_opto.t)
            ...
            out.shuf_peth_mean = [out.shuf_peth_mean; mean(big_peth,1)];
            out.shuf_peth_sd = [out.shuf_peth_sd; std(big_peth,1)];
            out.shuf_zpeth_mean = [out.shuf_zpeth_mean; mean(big_zpeth,1)];
            out.shuf_zpeth_sd = [out.shuf_zpeth_sd; std(big_zpeth,1)];
