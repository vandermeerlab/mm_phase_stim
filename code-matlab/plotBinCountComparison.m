...
for iF = 1:length(fbands)
    ...
    for iC = 1:size(summary.fr_r,1)
        plot(bin_counts, summary.fr_r(iC,:,iF), 'Color', c_list{iF});
    end
    mean_trend = squeeze(mean(summary.fr_r, 1));
    plot(bin_counts, mean_trend(:,iF), 'LineWidth', 2, 'Color', 'black');
    xlabel('Bin Count')
    ...
    for iC = 1:size(summary.fr_z,1)
        plot(bin_counts, summary.fr_z(iC,:,iF), 'Color', c_list{iF});
    end
    ...
    for iN = 1:length(this_sig)
        text(0.2*(iN-1),0.75, string(this_sig(iN)), 'FontSize', 16);
        text(0.2*(iN-1),0.25, string(this_nan(iN)), 'FontSize', 16);
    end
    yticks([0.25 0.75])
    yticklabels({'Ineligible', 'Count'});
    xticks([0:0.2:0.8])
...
function s_out = doStuff(s_in)
    ...
    bin_count = [5,7,10,15,20];
    for iC = 1:length(ExpKeys.goodCell)
        fn_prefix = extractBefore(ExpKeys.goodCell{iC}, '.t');
        this_fr_r = zeros(length(bin_count),4);
        this_fr_z = zeros(length(bin_count),4);
        this_response = 0;
        for iN = 1:length(bin_count)
            load(strcat(fn_prefix, strcat('_phase_response_', string(bin_count(iN)),'_bins.mat')));
