...
%         title(sprintf('Hilbert %d - % d Hz',fbands{iF}(1), fbands{iF}(2)));
% 
%         subplot(3,3, (iF-1)*3+2)
%         histogram(causal_phase(iF,:),phase_bins,'FaceColor',c_list{iF});
%         title(sprintf('Causal %d - % d Hz',fbands{iF}(1), fbands{iF}(2)));
%     end
% 
%     subplot(3,3,[3,6,9])
...
%     print(fig, '-dpng',  strcat('E:\Dropbox (Dartmouth College)\AnalysisResults\phase_stim_results\ArtifactEval\', fn_prefix, '-LFP-Report'));
%     close;
    ...
    while first < length(stim_on)
        ...
        first = first+5;
        print(fig, '-dpng',  strcat('E:\Dropbox (Dartmouth College)\AnalysisResults\phase_stim_results\STA_dump\', num2str(first)));
        close;
    end       
end
